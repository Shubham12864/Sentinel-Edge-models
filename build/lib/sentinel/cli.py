...
ROOT = None  # resolved lazily in _bootstrap()
T1 = T2 = T3 = None


def _bootstrap():
    """Resolve the application root (repo checkout or unpacked wheel install)."""
    global ROOT, T1, T2, T3
    if ROOT is not None:
        return
    try:
        from sentinel.approot import app_root
        ROOT = app_root()
    except Exception:
        ROOT = Path(__file__).resolve().parents[1]   # repo fallback
